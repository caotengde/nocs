...
* Under both **background-removed** and **background-retained** settings,  
  the two losses (**train_loss** and **sample_loss**) converge rapidly,  
  indicating that the latent velocity field and the final NOCS reconstruction  
  are effectively learned.

* The **background-removed** setting converges faster in the early stages.

* By **40 000 steps**, the **sample_loss** is almost identical in both settings,  
  showing that the final reconstruction accuracy differs very little.

* Moreover, when **background is retained**, the model not only learns  
  the **foreground mask prediction** well but also accurately recovers  
  the **NOCS map**, demonstrating that moderate background information  
  does not hinder the final pose estimation quality.
